    ...
    public static void onTick(PlayerTickEvent.Post event) {
        ...
        if (t >= SHAPE_TICKS) {
            ...
            if (!(level instanceof net.minecraft.server.level.ServerLevel sLevel)) {
                return; // bezpečná poistka – tvorbu ember robíme len na serveri
            }

            if (!sk.gothmur.mod.util.TinderHelper.makeEmber(sLevel, player, d.pos)) {
                // hráč nemá tinder → zruš a povedz mu to
                player.displayClientMessage(net.minecraft.network.chat.Component.translatable("msg.stone2steel.need_tinder"), true);
                playCancel(level, d.pos);                   // tvoj existujúci efekt „neúspech“
                ACTIVE.remove(player.getUUID());            // ak máš takú mapu sessionov
                return;
            }

// úspech – ember bol vyrobený (už je v inventári), zobraz hlášku
            player.displayClientMessage(net.minecraft.network.chat.Component.translatable("msg.stone2steel.ember_ok"), true);
